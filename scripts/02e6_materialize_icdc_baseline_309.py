...
Roster provenance
-----------------
The exact 309 case IDs were previously frozen by 02d2. Because older local
runs may have used slightly different result-directory/file names, this script
discovers ONLY roster-like files whose paths contain one of:
    "02d2", "parent_roster", "globalsearch"
and accepts a file only if it independently contains exactly the frozen
152/157/309 unique COTC021/COTC022 case-ID set.

If multiple exact candidate files are found, they must contain the identical
309-ID set or the script fails closed.
...
SCRIPT_VERSION = "02e6-materialize-icdc-baseline-309-v1-no-cli"
...
ROSTER_PATH_HINTS = ("02d2", "parent_roster", "globalsearch")
ROSTER_EXTENSIONS = {".json", ".jsonl", ".csv", ".tsv", ".txt"}
...
def discover_frozen_roster() -> Tuple[List[str], List[Dict[str, Any]]]:
    """
    Search only roster-like paths. Accept only exact 152/157/309 files.
    """
    candidates: List[Path] = []

    # Search results/data-like areas, not the entire repository contents.
    search_roots = [
        ROOT / "results",
        ROOT / "data",
        ROOT / "artifacts",
        ROOT / "method_contract",
    ]
    ...
    for search_root in search_roots:
        ...
        for path in search_root.rglob("*"):
            ...
            if not any(hint in lower_path for hint in ROSTER_PATH_HINTS):
                continue
            if path.suffix.lower() not in ROSTER_EXTENSIONS:
                continue
            rp = path.resolve()
    ...
    exact: List[Tuple[Path, Set[str]]] = []
    for path in sorted(candidates, key=lambda p: str(p)):
        ids = safe_read_roster_candidate(path)
        if exact_roster(ids):
            exact.append((path, ids))

    if not exact:
        inspected = [relative(p) for p in candidates[:50]]
        raise RuntimeError(
            "Could not locate an exact frozen 02d2 public roster containing "
            "152 COTC021 + 157 COTC022 = 309 unique case IDs.\n"
            "Only roster-like paths containing 02d2/parent_roster/globalsearch "
            "were inspected.\n"
            f"Candidate files inspected ({len(candidates)}): {inspected}"
        )

    canonical_set = exact[0][1]
    disagreements = [
        relative(path)
        for path, ids in exact
        if ids != canonical_set
    ]
    if disagreements:
        raise RuntimeError(
            "Multiple exact-size roster candidates disagree on case IDs: "
            + ", ".join(disagreements)
        )
