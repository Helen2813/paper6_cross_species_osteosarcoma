...
SCRIPT_VERSION = "03b-run-dog2-source-prognostic-gate-v1-no-cli"
...
def main() -> None:
    ...
    os_arm_status, os_arm_diag = arm_status(
        ...
        float(protocol["arm_to_arm_generalization"]["arm_pass"]["mean_directional_uno_c_min"]),
        float(protocol["arm_to_arm_generalization"]["arm_warn"]["mean_directional_uno_c_min"]),
        float(protocol["arm_to_arm_generalization"]["arm_pass"]["max_drop_from_random_cv_mean_uno_c"]),
    )
    ...
    combined_matrix = protocol.get("combined_gate_matrix") or {}
    combined_status = clean(combined_matrix.get(combined_key))

    if not combined_status:
        raise RuntimeError(
            f"Frozen combined gate matrix lacks key {combined_key!r}."
        )
