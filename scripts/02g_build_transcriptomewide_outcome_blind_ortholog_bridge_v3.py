...
SCRIPT_VERSION = "02g-build-transcriptomewide-outcome-blind-ortholog-bridge-v3-no-cli"
...
OUT_DIR = ROOT / "results" / "ortholog_bridge" / "02g_v3"
...
ATTR_RAW = OUT_DIR / "ensembl_biomart_attributes_raw.txt"
DATASET_AUDIT = OUT_DIR / "ensembl_biomart_dataset_audit.tsv"
DATASET_RAW = OUT_DIR / "ensembl_biomart_datasets_raw.txt"
...
ENSEMBL_MART = "https://mart.ensembl.org/biomart/martservice"
DOG_DATASET = "clfamiliaris_gene_ensembl"
ENSEMBL_MART_NAME = "ENSEMBL_MART_ENSEMBL"
ENSEMBL_VIRTUAL_SCHEMA = "default"
...
def run_biomart_xml_query(
    ...
    *,
    ...
    """
    Execute a BioMart XML query against the clean martservice endpoint.

    Primary route: POST form field query=<XML>.
    Fallback: GET ?query=<XML>.

    The returned route label is frozen in provenance.
    """
    post_error = ""
    try:
        text = http_post_text(
        ...
        if text.strip() and not text.lstrip().lower().startswith("query error"):
            return text, "POST_FORM_QUERY"
        post_error = text[:1000]
    except Exception as exc:
    ...
    try:
        ...
        if text.strip() and not text.lstrip().lower().startswith("query error"):
            return text, "GET_QUERY_PARAMETER"
        raise RuntimeError(f"GET BioMart query error/empty response: {text[:1000]}")
    except Exception as exc:
        raise RuntimeError(
            "BioMart XML query failed through both POST and GET routes. "
            f"POST diagnostic: {post_error}; GET diagnostic: {type(exc).__name__}: {exc}"
        ) from exc
...
def list_biomart_datasets(session: requests.Session) -> pd.DataFrame:
    text = http_get_text(
        session,
        ENSEMBL_MART,
        params={
            "type": "datasets",
            "mart": ENSEMBL_MART_NAME,
            "virtualSchema": ENSEMBL_VIRTUAL_SCHEMA,
            "requestid": "Paper6_02g",
        },
        timeout=120,
    )
    DATASET_RAW.write_text(text, encoding="utf-8", newline="\n")
    ...
    for line in text.splitlines():
        ...
        rows.append(
            {
                "dataset_type": parts[0] if len(parts) > 0 else "",
    ...
    if frame.empty or "dataset" not in frame.columns:
        raise RuntimeError(
            "BioMart dataset inventory was empty/unparseable. "
            f"Raw response saved to {DATASET_RAW.relative_to(ROOT)}"
        )
    frame.to_csv(DATASET_AUDIT, sep="\t", index=False)
    return frame


def resolve_current_dog_dataset(session: requests.Session) -> str:
    frame = list_biomart_datasets(session)
    datasets = set(frame["dataset"].astype(str).str.strip())

    if DOG_DATASET in datasets:
        return DOG_DATASET

    dog_candidates = frame[
        frame["description"].astype(str).str.contains(
            r"\bDog\b|Canis lupus familiaris",
            case=False,
            regex=True,
            na=False,
        )
        | frame["dataset"].astype(str).str.contains(
            r"familiaris|canis|dog",
            case=False,
            regex=True,
            na=False,
        )
    ...
    candidate_text = "; ".join(
        f"{r.dataset} [{r.description}]"
        for r in dog_candidates.itertuples(index=False)
    )
    raise RuntimeError(
        f"Expected current Ensembl dog dataset {DOG_DATASET!r} was not found. "
        f"Dog-like candidates: {candidate_text or '<none>'}. "
        f"Dataset inventory saved to {DATASET_AUDIT.relative_to(ROOT)}"
    )


def list_biomart_attributes(
    session: requests.Session,
    dataset: str,
) -> pd.DataFrame:
    text = http_get_text(
    ...
    ATTR_RAW.write_text(text, encoding="utf-8", newline="\n")
    ...
    if frame.empty or "name" not in frame.columns:
        raise RuntimeError(
            "BioMart attribute inventory was empty/unparseable. "
            f"Raw response saved to {ATTR_RAW.relative_to(ROOT)}"
        )

    frame.to_csv(ATTR_AUDIT, sep="\t", index=False)
    return frame


def build_biomart_query(attributes: Sequence[str], dataset: str) -> str:
    attribute_xml = "\n".join(
...
def fetch_or_reuse_current_snapshot(
    ...
    if SNAPSHOT_RAW.exists() and SNAPSHOT_META.exists():
        ...
        if not clean(meta.get("biomart_dataset")):
            raise RuntimeError(
                "Existing current-Ensembl snapshot metadata lacks biomart_dataset."
            )
    ...
    resolved_dataset = resolve_current_dog_dataset(session)
    attributes = list_biomart_attributes(session, resolved_dataset)
    available = set(attributes["name"].astype(str).str.strip())

    missing = [x for x in REQUIRED_CURRENT_ATTRIBUTES if x not in available]
    if missing:
        human_homology_candidates = sorted(
            x for x in available
            if (
                "homolog" in x.lower()
                or "ortholog" in x.lower()
                or "hsapiens" in x.lower()
                or "human" in x.lower()
            )
        )
        core_gene_candidates = sorted(
            x for x in available
            if (
                "ensembl_gene" in x.lower()
                or "gene_name" in x.lower()
                or "external_gene" in x.lower()
            )
        )
        raise RuntimeError(
            f"Current {resolved_dataset} BioMart is missing exact required attribute names: {missing}. "
            f"Observed core-gene candidates: {core_gene_candidates[:30]}. "
            f"Observed human/homology candidates: {human_homology_candidates[:60]}. "
            f"Full inventory saved to {ATTR_AUDIT.relative_to(ROOT)} and "
            f"{ATTR_RAW.relative_to(ROOT)}."
        )
    ...
    query = build_biomart_query(selected, resolved_dataset)
    text, query_transport = run_biomart_xml_query(
        session,
        ENSEMBL_MART,
        query,
        timeout=300,
    ...
    meta = {
        ...
        "biomart_dataset": resolved_dataset,
        "biomart_mart": ENSEMBL_MART_NAME,
        "biomart_virtual_schema": ENSEMBL_VIRTUAL_SCHEMA,
        ...
        "biomart_query_transport": query_transport,
...
def main() -> None:
    ...
    print(f"  BioMart endpoint: {ENSEMBL_MART}")
    print(
        "  snapshot source: "
        + (
    ...
    contract = {
        ...
        "fresh_primary_mapping_snapshot": {
            ...
            "biomart_dataset": current_meta.get("biomart_dataset"),
            "biomart_mart": current_meta.get("biomart_mart"),
            "biomart_virtual_schema": current_meta.get("biomart_virtual_schema"),
    ...
    paths = [
        SNAPSHOT_RAW,
        SNAPSHOT_META,
        DATASET_RAW if DATASET_RAW.exists() else None,
        DATASET_AUDIT if DATASET_AUDIT.exists() else None,
        ATTR_RAW if ATTR_RAW.exists() else None,
