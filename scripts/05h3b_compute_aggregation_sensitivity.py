...
Expected placement:
    scripts/05h3b_compute_aggregation_sensitivity.py

Run:
    python scripts/05h3b_compute_aggregation_sensitivity.py
"""
...
SCRIPT_VERSION = "05h3b-compute-aggregation-sensitivity-v1-scenario-id-core-resolver-no-cli"
...
def project_root() -> Path:
    ...
    if p.parent.name.lower() != "scripts":
        raise RuntimeError(
            "Place 05h3b_compute_aggregation_sensitivity.py in the repository scripts/ directory."
        )
...
OUT_DIR = ROOT / "method_contract" / "05h3b_aggregation_sensitivity"
WORK_DIR = ROOT / "method_contract" / ".05h3b_aggregation_sensitivity_work"
...
def resolve_scenario_class_from_frozen_id(x: Any) -> str:
    """
    Resolve CORE versus STRESS from the immutable frozen scenario identifier.

    The 05d `family` column is NOT a binary CORE/STRESS flag. It contains
    descriptive design-family labels such as CORE_PHASE_DIAGRAM and
    SHIFT_CENSOR_STRESS. The binary pre-stress/core membership is instead
    encoded explicitly in the authoritative frozen scenario IDs:
        CORE_0000 ... CORE_0035
        STRESS_0036 ... STRESS_0179

    This parser does not inspect outcome/model metric values. Exact membership
    sets are verified immediately after parsing.
    """
    s = str(x).strip().upper()
    if re.fullmatch(r"CORE_\\d{4}", s):
        return "CORE"
    if re.fullmatch(r"STRESS_\\d{4}", s):
        return "STRESS"
    raise RuntimeError(
        f"Unexpected frozen scenario_id for CORE/STRESS resolution: {x!r}"
    )
...
def load_scenario_design_and_metrics() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    design["scenario_class"] = design["scenario_id"].map(
        resolve_scenario_class_from_frozen_id
    )
    ...
    # The 05h0 guardrail is exact membership, not merely a count. Verify the
    # immutable frozen CORE/STRESS ID sets byte-for-byte in semantic terms.
    observed_core_ids = set(
        design.loc[design["scenario_class"] == "CORE", "scenario_id"].astype(str)
    )
    observed_stress_ids = set(
        design.loc[design["scenario_class"] == "STRESS", "scenario_id"].astype(str)
    )
    expected_core_ids = {f"CORE_{i:04d}" for i in range(0, 36)}
    expected_stress_ids = {f"STRESS_{i:04d}" for i in range(36, 180)}
    ...
    if observed_core_ids != expected_core_ids:
        ...
        raise RuntimeError(
            "Frozen CORE scenario membership changed. "
            f"Missing={missing[:10]}, extra={extra[:10]}"
        )
    if observed_stress_ids != expected_stress_ids:
        missing = sorted(expected_stress_ids - observed_stress_ids)
        extra = sorted(observed_stress_ids - expected_stress_ids)
        raise RuntimeError(
            "Frozen STRESS scenario membership changed. "
            f"Missing={missing[:10]}, extra={extra[:10]}"
        )

    n_core = len(observed_core_ids)
    n_stress = len(observed_stress_ids)
    if n_core != EXPECTED_CORE_SCENARIOS or n_stress != EXPECTED_STRESS_SCENARIOS:
        raise RuntimeError(
            f"Frozen CORE/STRESS counts changed: {n_core}/{n_stress}."
        )

    # `family` remains provenance-bearing descriptive design metadata. It is not
    # coerced into a binary flag. Require it to be present and stable per scenario.
    if design["family"].isna().any():
        raise RuntimeError("Missing descriptive frozen `family` metadata.")
    ...
    if metrics[["scenario_class", "regime", "replicates_design"]].isna().any().any():
...
def estimand_weights(design: pd.DataFrame) -> dict[str, dict[str, float]]:
    ...
    # Core-only: exactly the 36 pre-stress scenarios, resolved from frozen scenario IDs.
    core_ids = list(
        design.loc[
...
def verify_original_against_05h2b(
    ...
    for model in PRIMARY_MODELS:
        ...
        for pcol, hcol in pairs:
            ...
            checks.append(
                {
                    "check": "05h3b_frozen_original_vs_05h2b",
                    "model": model,
                    "metric": pcol,
                    "observed": observed,
                    "expected": expected,
...
def build_readme(
    ...
    lines = [
        ...
        "--------------------",
        "180 total scenarios; exact CORE_0000..CORE_0035 membership; exact STRESS_0036..STRESS_0179 membership.",
        ...
        "--------------------------",
        "CORE/STRESS membership is resolved from the immutable frozen scenario IDs, not from the descriptive `family` label.",
...
def main() -> None:
    ...
    if OUT_DIR.exists():
        raise RuntimeError(
            f"Final 05h3b output already exists; refusing overwrite: {OUT_DIR}"
        )
    if WORK_DIR.exists():
    ...
    try:
        ...
        print("  CORE/STRESS resolver: authoritative frozen scenario_id membership")
        print("  descriptive `family` metadata retained but not coerced to binary class")
        print("  regime resolver: authoritative frozen transfer_regime metadata")
        print(f"  scenarios: {len(design)}/180")
        print(
            "  CORE/STRESS: "
            f"{int((design['scenario_class'] == 'CORE').sum())}/"
            f"{int((design['scenario_class'] == 'STRESS').sum())}"
        )
        ...
        print("-" * 118)
        print("05h3b AGGREGATION-SENSITIVITY POINT ESTIMATES")
        print("-" * 118)
        show = points[
            [
                "estimand",
        ...
        print("=" * 118)
        print("05h3b post-HOLD aggregation sensitivity: PASS")
        print("=" * 118)
...
if __name__ == "__main__":
    try:
        ...
        print()
        print("=" * 118)
        print("05h3b post-HOLD aggregation sensitivity: FAIL")
