...
SCRIPT_VERSION = (
    "05f3b-audit-target-os-preexisting-complete-case-policy-v2-zero-time-reconciliation-no-cli"
)
AUDIT_VERSION = (
    "paper6-postopening-target-os-complete-case-reconciliation-v2-zero-time"
)
...
# Outputs.
# ---------------------------------------------------------------------------
OUT_DIR = ROOT / "results" / "human_posthold_descriptive" / "05f3b_v2"
...
def parse_nonmissing_time(
    ...
    """
    Reconcile endpoint COMPLETENESS exactly as the pre-existing Paper-4
    complete-case analysis did: nonmissing numeric time is present even when
    it equals zero.

    Zero time is NOT silently declared scientifically ideal. It is recorded
    separately for the later execution amendment. Negative/nonfinite/non-
    numeric nonblank values remain FAIL_CLOSED.
    """
    text = str(raw).strip()
    ...
    if value < 0:
        return False, None, f"INVALID_NEGATIVE:{value}"

    if value == 0:
        return True, 0.0, "PARSED_ZERO_TIME"

    return True, float(value), "PARSED_POSITIVE_TIME"
...
def main() -> None:
    ...
    print("  recovery from 05f3b v1 overly-strict time>0 check: YES")
    print("  v1 model fitting occurred: NO")
    print("  zero time treated as observed for completeness only: YES")
    print()
    print("Read scope:")
    ...
    n_zero_time_complete = int(len(zero_time_complete))
    ...
    print(f"Complete-case zero-time rows: {n_zero_time_complete}")
    if n_zero_time_complete:
        print("Zero-time complete-case rows:")
        for row in zero_time_complete.itertuples(index=False):
            print(
                f"  {row.sample_id}: "
                f"os_event={int(row.os_event_parsed)}, "
                f"os_time_days={float(row.os_time_days_parsed):.1f}"
            )
    print()
    ...
    reconciliation = {
        ...
        "postopening_amendment": {
            ...
            "eligibility_rule": (
                "reproduce pre-existing Paper4 complete-case policy literally: "
                "include a TARGET expression case iff BOTH frozen primary "
                "os_event and os_time_days are present and parseable; numeric "
                "zero time counts as present because upstream script23 used "
                "dropna(), not a >0 filter; blank endpoint field -> incomplete; "
                "negative/nonfinite/nonnumeric nonblank time -> FAIL_CLOSED"
            ),
            "zero_time_policy": (
                "zero time is retained for endpoint-completeness reconciliation "
                "and explicitly flagged; no epsilon shift or exclusion is "
        ...
        "current_TARGET_primary_endpoint": {
            ...
            "events": n_events,
            "censored": n_censored,
            "zero_time_complete_cases": n_zero_time_complete,
        ...
        "next": (
            "05f3c technical execution-policy audit/amendment: use the exact "
            "pre-existing 86-case OS-complete roster and explicitly reconcile "
            "any zero-time row with the authoritative Paper4/sksurv execution "
            "semantics before model execution; no outcome imputation, no "
            "prediction-based exclusion, and no model/branch tuning."
        ),
    }
    ...
    print("Next:")
    print(
        "  05f3c reconcile execution on the exact 86-case upstream OS roster, "
        "including explicit zero-time semantics, before any model run."
    )
    print("=" * 120)
    print(
