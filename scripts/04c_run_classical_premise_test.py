...
SCRIPT_VERSION = "04c-run-classical-premise-test-v1-no-cli"
...
EVENT_MATCHED_DRAWS = OUT_DIR / "event_matched_dog_draw_metrics.tsv"
            ...
            [
                {
                    ...
                    "model": "B0",
                    "n_test": len(test_idx),
                    "time_grid_n": len(grid),
                    "time_grid_min": float(grid[0]),
                    "time_grid_max": float(grid[-1]),
                    "ibs": ibs_b0,
                },
                {
                    "repeat": repeat,
...
def run_event_matched_repeat(
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer):
        ...
        for draw in range(EVENT_MATCHED_DRAWS):
            draw_seed = (
                base_seed
                + repeat * 10000
                + outer_fold * 100
...
def main() -> None:
    ...
    ibs_b0 = weighted_mean_ibs(primary_ibs, "B0")
    ibs_b4 = weighted_mean_ibs(primary_ibs, "B4")
    ...
    em_draws.to_csv(EVENT_MATCHED_DRAWS, sep="\t", index=False)
    ...
    write_json(
        EVENT_MATCHED_RESULT,
        {
            ...
            "draws_per_outer_fold": EVENT_MATCHED_DRAWS,
        },
    )
