...
SCRIPT_VERSION = (
    "05f3d-run-exact-target86-after-audited-zero-time-amendment-v1-no-cli"
)
EXECUTION_VERSION = (
    "paper6-target86-postopening-audited-frozen-execution-v1"
)
...
RUNNER_V3 = (
    RUNNER_DIR
    / "frozen_TARGET86_evaluation_runner_v3.py"
)
...
def create_exact_runner_v3() -> Dict[str, Any]:
    original_text = F2C_RUNNER.read_text(
        encoding="utf-8"
    )
    ...
    observed_counts = {
        "n_88_assertion_occurrences": original_text.count(
            old_n
        ),
        "shape_88_assertion_occurrences": original_text.count(
            old_shape
        ),
        "time_le_zero_assertion_occurrences": original_text.count(
            old_time
        ),
    }
    ...
    if (
        ...
        raise RuntimeError(
            "Runner v2 patch-site counts differ from frozen 05f3c audit: "
            f"{observed_counts}"
        )
    ...
    # Reconstruct a second time independently and require exact bytes.
    independent = original_text
    ...
    if (
        ...
        raise RuntimeError(
            "Independent exact runner-patch reconstruction differs."
        )

    RUNNER_V3.write_text(
        patched,
        encoding="utf-8",
    )

    if (
        RUNNER_V3.read_bytes()
        != patched.encode(
            "utf-8"
        )
    ):
        raise RuntimeError(
            "Runner v3 disk replay differs from constructed bytes."
        )

    # Verify no unauthorized textual differences by reverse patching v3.
    reverse = RUNNER_V3.read_text(
        encoding="utf-8"
    )
    ...
    if (
        ...
        raise RuntimeError(
            "Runner v3 expected-n patch is not unique."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Runner v3 shape patch is not unique."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Runner v3 time patch is not unique."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Runner v3 contains unauthorized changes beyond the three audited substitutions."
        )
    ...
    patch_audit = {
        ...
        "patch_site_counts": observed_counts,
        "reverse_patch_reproduces_runner_v2_byte_exact": True,
...
def main() -> None:
    ...
    print("Runner v2 -> v3 exact amendment: PASS")
    print(
    ...
    print(
        "  reverse patch reproduces v2 byte-exact: PASS"
    )
    print(
    ...
    summary = {
        ...
        "postopening_amendment_status": {
            ...
            "runner_v2_to_v3_exact_patch": "PASS",
    ...
    print("=" * 120)
    print(
        "05f3d: PASS_TARGET86_POSTOPENING_AUDITED_FROZEN_DESCRIPTIVE_EVALUATION_COMPLETE"
    )
    print("=" * 120)
...
if __name__ == "__main__":
    try:
        ...
        print(
            "05f3d exact TARGET86 frozen execution: FAIL",
            file=sys.stderr,
        )
        print(
