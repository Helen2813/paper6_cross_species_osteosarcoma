...
SCRIPT_VERSION = "02e9-run-expression-gate-zero-v2-no-cli"

ROOT = Path(__file__).resolve().parents[1]

# Script-00 upstream provenance. Paper 6 verifies upstream Paper 4 assets
# without copying them into the Paper 6 repository.
...
def _path_candidates_from_string(
    value: str,
    filename: str,
    relative_path: str,
) -> List[Path]:
    """
    Convert provenance strings into plausible filesystem candidates without
    trusting any one manifest field name.
    """
    ...
    if filename.lower() not in normalized.lower():
        return candidates

    p = Path(normalized)

    # Absolute path recorded by script-00 provenance.
    if p.is_absolute():
        candidates.append(p)

    # Project-root-like provenance string.
    if normalized.lower().endswith(filename.lower()):
        candidates.append(p)

    # If a provenance cell contains a parent/project directory rather than a
    # clean path, also try appending the frozen project-relative asset path.
    lower = normalized.lower()
    if "paper4_sarcoma_dog" in lower and not lower.endswith(filename.lower()):
        candidates.append(p / Path(relative_path))
...
def expression_path_from_contract(contract: Dict[str, Any]) -> Path:
    """
    Resolve the locked DOG² expression asset from its upstream Paper 4
    provenance.

    Paper 6 script 00 deliberately did NOT copy upstream assets. Therefore
    `data/processed/...` is relative to the authoritative Paper 4 project,
    not necessarily to the Paper 6 project root.

    Resolution is fail-closed: every candidate must exist and match the exact
    frozen 02e8 SHA256 before it is accepted.
    """
    asset = contract.get("expression_asset") or {}
    relative_path = clean(asset.get("expression_relative_path"))
    expected_sha = clean(asset.get("expression_sha256"))

    if not relative_path:
        raise RuntimeError("02e8 contract lacks expression relative path.")
    if len(expected_sha) != 64:
        raise RuntimeError("02e8 contract lacks a valid expression SHA256.")

    filename = Path(relative_path).name

    candidates: List[Path] = []

    # 1) Script-00 lock: inspect all path-like provenance strings, independent
    #    of the exact historical field names.
    if UPSTREAM_LOCK.exists():
        upstream_lock = read_json(UPSTREAM_LOCK)
        dog2_asset = (upstream_lock.get("assets") or {}).get("dog2_expression")
        if isinstance(dog2_asset, dict):
            for value in _collect_strings(dog2_asset):
                candidates.extend(
                    _path_candidates_from_string(
                        value,
                        filename=filename,
    ...
    # 2) Script-00 manifest: inspect the dog2_expression row(s), again without
    #    assuming a specific path-column name.
    if UPSTREAM_MANIFEST.exists():
        try:
            manifest = pd.read_csv(UPSTREAM_MANIFEST, dtype=str).fillna("")
            mask = pd.Series(False, index=manifest.index)
            for col in manifest.columns:
                mask = mask | (
                    manifest[col].astype(str).str.contains(
                        "dog2_expression",
                        case=False,
                        regex=False,
                    )
                )
            matching = manifest.loc[mask]
            ...
            raise RuntimeError(
                f"Could not inspect script-00 upstream manifest: {exc}"
            ) from exc

    # 3) Canonical sibling-repository layout used by the frozen upstream Paper 4
    #    project. This is NOT a new source; it is the physical location of the
    #    already hash-locked asset.
    candidates.append(
        ROOT.parent
        / "paper4_sarcoma_dog"
        / Path(relative_path)
    ...
    # 4) Preserve the Paper-6-local candidate only as a final compatibility
    #    option in case a future checkout actually materializes the locked file.
    candidates.append(ROOT / Path(relative_path))

    # Deduplicate while preserving provenance-first preference.
    ...
    existing_wrong_hash: List[Tuple[str, str]] = []
    ...
    for candidate in unique_candidates:
        ...
        if actual_sha.lower() == expected_sha.lower():
            print("Resolved locked DOG² expression upstream:")
            print(f"  physical path: {candidate}")
            print(f"  SHA256: {actual_sha}")
            return candidate

        existing_wrong_hash.append((str(candidate), actual_sha))

    if existing_wrong_hash:
        details = "; ".join(
            f"{path} [sha256={sha}]"
            for path, sha in existing_wrong_hash
        )
        raise RuntimeError(
            "Found DOG² expression filename candidate(s), but none matched the "
            f"frozen SHA256 {expected_sha}. Refusing to read them. {details}"
        )

    checked = [str(path) for path in unique_candidates]
    raise FileNotFoundError(
        "Could not resolve the hash-locked DOG² expression asset from script-00 "
        "upstream provenance or the canonical sibling Paper 4 repository. "
        f"Frozen relative path: {relative_path}. "
