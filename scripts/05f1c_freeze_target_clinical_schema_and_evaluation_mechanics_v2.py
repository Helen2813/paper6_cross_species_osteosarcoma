...
SCRIPT_VERSION = (
    "05f1c-freeze-target-clinical-schema-and-evaluation-mechanics-v2-id-schema-keyfix-no-cli"
)
CONTRACT_VERSION = (
    "paper6-posthold-target-clinical-schema-evaluation-v2-id-schema-keyfix"
)
...
# Outputs.
# ---------------------------------------------------------------------------
OUT_DIR = ROOT / "results" / "human_posthold_descriptive" / "05f1c_v2"
...
# ---------------------------------------------------------------------------
ID_ALIAS_PRIORITY = [
    # Outcome-free 05f1c0 probe: exact 88/88 TARGET expression match.
    "case_submitter_id",
    "casesubmitterid",
    "targetusi",
    "sampleid",
...
OS_EVENT_ALIAS_PRIORITY = [
    # Outcome-free 05f1c0 header probe: standardized primary OS event column.
    "os_event",
    "osevent",
    "osstatus",
...
OS_TIME_ALIAS_PRIORITY = [
    # Outcome-free 05f1c0 header probe: standardized primary OS time column.
    "os_time_days",
    "ostimedays",
    "ostime",
    "osdays",
...
# Outcome-free 05f1c0 probe findings. These are schema facts, not outcome values.
PROBED_CLINICAL_ASSET_ROLE = "target_clinical"
PROBED_CLINICAL_RELATIVE_PATH = (
    "data/processed/human_validation/TARGET_OS_clinical_standardized.csv"
)
PROBED_ID_COLUMN = "case_submitter_id"
PROBED_OS_EVENT_COLUMN = "os_event"
PROBED_OS_TIME_COLUMN = "os_time_days"
PROBED_ID_MATCH_N = 88
PROBED_ID_TRANSFORM = "UPPER_STRIPPED_EXACT"
...
def main() -> None:
    ...
    print("  Technical recovery from 05f1c v1 ID-alias mismatch: YES")
    print("  05f1c0 outcome-free identifier/header probe used: YES")
    ...
    # Technical recovery guardrail: the v2 fix is permitted only for the
    # exact locked standardized TARGET clinical asset probed outcome-free.
    if clinical_role != PROBED_CLINICAL_ASSET_ROLE:
        raise RuntimeError(
            f"Clinical asset role differs from 05f1c0 probe: "
            f"{clinical_role!r} != {PROBED_CLINICAL_ASSET_ROLE!r}"
        )
    if clean(clinical_asset.get("relative_path")) != PROBED_CLINICAL_RELATIVE_PATH:
        raise RuntimeError(
            "Clinical asset path differs from outcome-free 05f1c0 probe."
        )
    ...
    # Technical schema identity proven by 05f1c0 without reading outcomes.
    if id_column != PROBED_ID_COLUMN:
        raise RuntimeError(
            f"Selected clinical ID column {id_column!r} differs from "
            f"outcome-free probe result {PROBED_ID_COLUMN!r}."
        )

    if os_schema["schema_type"] != "PRECOMPUTED_OS_EVENT_PLUS_TIME":
        raise RuntimeError(
            "Expected standardized precomputed OS event+time schema after "
            "outcome-free header probe."
        )

    if os_schema["event_column"] != PROBED_OS_EVENT_COLUMN:
        raise RuntimeError(
            f"Selected OS event column {os_schema['event_column']!r} differs "
            f"from outcome-free probe header {PROBED_OS_EVENT_COLUMN!r}."
        )

    if os_schema["time_column"] != PROBED_OS_TIME_COLUMN:
        raise RuntimeError(
            f"Selected OS time column {os_schema['time_column']!r} differs "
            f"from outcome-free probe header {PROBED_OS_TIME_COLUMN!r}."
        )
    ...
    if id_transform_name != PROBED_ID_TRANSFORM:
        raise RuntimeError(
            f"Identifier transform {id_transform_name!r} differs from "
            f"outcome-free probe result {PROBED_ID_TRANSFORM!r}."
        )

    if len(id_mapping) != PROBED_ID_MATCH_N:
        raise RuntimeError(
            f"Identifier mapping count {len(id_mapping)} differs from "
            f"outcome-free probe result {PROBED_ID_MATCH_N}."
        )
    ...
    endpoint_schema_payload = {
        ...
        "technical_recovery": {
            ...
            "failure_stage": "non-outcome expression-to-clinical identifier mapping",
            "failure_reason": (
                "v1 alias priority selected case_id UUID instead of case_submitter_id"
            ),
            "05f1c0_probe_outcomes_read": False,
            "probe_result_ID_column": PROBED_ID_COLUMN,
            "probe_result_ID_transform": PROBED_ID_TRANSFORM,
            "probe_match": "88/88 unique exact",
            "probe_header_OS_event_column": PROBED_OS_EVENT_COLUMN,
            "probe_header_OS_time_column": PROBED_OS_TIME_COLUMN,
    ...
    summary = {
        ...
        "technical_recovery": {
            "from_05f1c_v1": True,
            "outcome_values_read_before_recovery": False,
            "identifier_probe_script": (
                "05f1c0-probe-target-clinical-identifier-schema-v1-no-cli"
            ),
            "identifier_column": PROBED_ID_COLUMN,
            "identifier_transform": PROBED_ID_TRANSFORM,
            "identifier_match": "88/88 unique",
            "OS_event_column_header_only": PROBED_OS_EVENT_COLUMN,
            "OS_time_column_header_only": PROBED_OS_TIME_COLUMN,
            "scientific_contract_changed": False,
        },
    ...
    print("  05f1c0 probe exact match: 88/88 PASS")
    print(
    ...
    print("  v1 selected UUID case_id and failed before outcome access")
    print("  v2 uses outcome-free probed case_submitter_id: YES")
