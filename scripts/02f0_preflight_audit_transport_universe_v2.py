...
SCRIPT_VERSION = "02f0-preflight-audit-transport-universe-v2-no-cli"
...
OUT_DIR = ROOT / "results" / "transport_contract" / "02f0_preflight_audit_v2"
...
def main() -> None:
    ...
    # Exact old evidence filenames are diagnostic only: historical scripts may have
    # renamed or omitted individual optional benchmark inputs.  They must not make
    # the provenance audit fail as long as the actual master->ortholog->QC->02f0
    # lineage is exact and the master table is demonstrably outcome/model aware.
    lineage_role_mask = source_lineage["evidence_role"].ne(
        "outcome_or_model_derived_input_to_master_evidence_universe"
    )
    outcome_input_mask = ~lineage_role_mask

    critical_lineage_tokens_present = bool(
        source_lineage.loc[lineage_role_mask, "token_present"].all()
    )
    ...
    checks = {
        ...
        "critical_master_to_ortholog_to_qc_to_02f0_lineage_tokens_present": critical_lineage_tokens_present,
    ...
    critical_checks = [
        ...
        "critical_master_to_ortholog_to_qc_to_02f0_lineage_tokens_present",
    ...
    audit = {
        ...
        "critical_source_lineage_tokens_present": critical_lineage_tokens_present,
        ...
        "interpretation": {
            "measured_fact": (
                "The row universe of the Paper-4 ortholog-QC table used by Paper-6 02f0 "
                "is the same gene universe as the Paper-4 RNA master evidence table, whose "
                "construction source explicitly consumes DFI/OS and model-selection evidence."
            ),
            "consequence": (
    ...
    print()
    print("Source-code lineage:")
    print(
        "  critical master->ortholog->QC->02f0 filename lineage present: "
        f"{critical_lineage_tokens_present}"
    )
    print(
        "  historical outcome/model input filename tokens present "
        f"[diagnostic only]: {outcome_input_tokens_present_n}/{outcome_input_tokens_total_n}"
    )
    if not missing_diagnostic_tokens.empty:
        print("  missing historical filename tokens [NON-GATING]:")
