...
Expected placement:
    scripts/05h5c_compute_target_km_ibs_reference.py

Run:
    python scripts\\05h5c_compute_target_km_ibs_reference.py
"""
...
SCRIPT_VERSION = "05h5c-compute-target-km-ibs-reference-v1-frozen-mechanics-no-cli"
...
def project_root() -> Path:
    ...
    if p.parent.name.lower() != "scripts":
        raise RuntimeError(
            "Place 05h5c_compute_target_km_ibs_reference.py "
...
OUT_DIR = ROOT / "method_contract" / "05h5c_target_km_ibs_reference"
WORK_DIR = ROOT / "method_contract" / ".05h5c_target_km_ibs_reference_work"
...
def main() -> None:
    ...
    if OUT_DIR.exists():
        raise RuntimeError(
            f"05h5c final output already exists; refusing overwrite: {OUT_DIR}"
        )
    if WORK_DIR.exists():
    ...
    try:
        ...
        print("-" * 118)
        print("05h5c TARGET NO-COVARIATE KM IBS REFERENCE")
        print("-" * 118)
        ...
        for item in manifest["outputs"]:
            ...
            if sha256_file(p) != item["sha256"]:
                raise RuntimeError(
                    f"05h5c output changed before finalization: {p.name}"
                )

        os.replace(WORK_DIR, OUT_DIR)

        print("=" * 118)
        print("05h5c TARGET KM-IBS null reference: PASS")
        print("=" * 118)
...
if __name__ == "__main__":
    try:
        ...
        print()
        print("=" * 118)
        print("05h5c TARGET KM-IBS null reference: FAIL")
