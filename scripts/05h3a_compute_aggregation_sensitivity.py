...
Expected placement:
    scripts/05h3a_compute_aggregation_sensitivity.py

Run:
    python scripts/05h3a_compute_aggregation_sensitivity.py
"""
...
SCRIPT_VERSION = "05h3a-compute-aggregation-sensitivity-v1-family-label-parser-fix-no-cli"
...
def project_root() -> Path:
    ...
    if p.parent.name.lower() != "scripts":
        raise RuntimeError(
            "Place 05h3a_compute_aggregation_sensitivity.py in the repository scripts/ directory."
        )
...
OUT_DIR = ROOT / "method_contract" / "05h3a_aggregation_sensitivity"
WORK_DIR = ROOT / "method_contract" / ".05h3a_aggregation_sensitivity_work"
...
def normalize_family(x: Any) -> str:
    """
    Normalize the already-frozen scenario-family metadata.

    05h3 v1 assumed short labels such as CORE/STRESS, but the authoritative
    frozen 05d table uses descriptive labels such as CORE_PHASE_DIAGRAM.
    This is an implementation/parser correction only: the family assignment is
    still taken from the frozen `family` metadata, never inferred from outcomes
    or metric values. Scenario IDs and exact 36/144 counts remain independent
    structural cross-checks below.
    """
    s = str(x).strip().upper()
    ...
    # Authoritative descriptive frozen-family forms.  Use semantic family
    # prefixes rather than enumerating one fragile display label.
    if s.startswith("CORE_"):
        return "CORE"
    if s.startswith("STRESS_"):
        return "STRESS"

    raise RuntimeError(
        "Unexpected frozen scenario family label after the prespecified "
        f"CORE_/STRESS_ parser correction: {x!r}"
    )
...
def verify_original_against_05h2b(
    ...
    for model in PRIMARY_MODELS:
        ...
        for pcol, hcol in pairs:
            ...
            checks.append(
                {
                    "check": "05h3a_frozen_original_vs_05h2b",
                    "model": model,
                    "metric": pcol,
                    "observed": observed,
                    "expected": expected,
...
def main() -> None:
    ...
    if OUT_DIR.exists():
        raise RuntimeError(
            f"Final 05h3a output already exists; refusing overwrite: {OUT_DIR}"
        )
    if WORK_DIR.exists():
    ...
    try:
        ...
        print("-" * 118)
        print("05h3a AGGREGATION-SENSITIVITY POINT ESTIMATES")
        print("-" * 118)
        show = points[
            [
                "estimand",
        ...
        print("=" * 118)
        print("05h3a post-HOLD aggregation sensitivity: PASS")
        print("=" * 118)
...
if __name__ == "__main__":
    try:
        ...
        print()
        print("=" * 118)
        print("05h3a post-HOLD aggregation sensitivity: FAIL")
